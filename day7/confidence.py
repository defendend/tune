...
Four orthogonal layers stacked on top of a single OpenAI Chat Completions
call:

  1. Constraint check  (free)
     The response must contain all 4 required sections, the verdict word
     must be one of {Sufficient, Insufficient}, and Insufficient verdicts
     may have at most 3 questions. If broken → status=FAIL immediately.

  2. Logprob scoring  (free, piggybacks on the primary call)
     We ask for `logprobs=True, top_logprobs=5`. After "Verdict:" we
     aggregate top-k tokens into the canonical {Sufficient, Insufficient}
     distribution. `margin = top - second` is our self-reported certainty.
     margin < LOW_THR → FAIL.

  3. Self-check  (one extra short call, ~5 tokens out)
     A separate "judge" prompt that sees the description, the verdict
     produced, and the reasoning, and answers exactly YES or NO. We pull
     P(YES) from the first-token logprobs. This is a SEMANTIC signal,
     orthogonal to step 2 — it catches cases where the model is
     confidently wrong on subjective judgments. Status=OK requires BOTH
     margin ≥ HIGH_THR AND p_yes ≥ SELFCHECK_HIGH.

  4. Redundancy / self-consistency  (two extra calls, T=0.7)
     Only if either step 2 or 3 was borderline. We re-run the prompt
     twice with temperature 0.7 and majority-vote across all 3 verdicts.
     3/3 agree → upgrade to OK; 2/3 → keep UNSURE; otherwise FAIL.

The escalation is intentional: stages 1 and 2 are free; the self-check
adds ~10% cost; redundancy only fires for the small slice that needed it.
"""
